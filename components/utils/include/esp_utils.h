...
#define ESP_MALLOC(size) ({                                                                \
    ...
    if (!ptr)                                                                              \
    {                                                                                      \
        ESP_LOGE(TAG, "[%s, %d] <ESP_ERR_NO_MEM> Malloc size: %d, ptr: %p, heap free: %d", \
                 __func__, __LINE__, (int)size, ptr, esp_get_free_heap_size());            \
...
#define ESP_CALLOC(n, size) ({                                                             \
    ...
    if (!ptr)                                                                              \
    {                                                                                      \
        ESP_LOGW(TAG, "[%s, %d] <ESP_ERR_NO_MEM> Calloc size: %d, ptr: %p, heap free: %d", \
                 __func__, __LINE__, (int)(n) * (size), ptr, esp_get_free_heap_size());    \
        return ESP_ERR_NO_MEM;                                                             \
    }                                                                                      \
    ptr;                                                                                   \
})
...
#define ESP_REALLOC(ptr, size) ({                                                               \
    ...
    if (!new_ptr)                                                                               \
    {                                                                                           \
        ESP_LOGW(TAG, "[%s, %d] <ESP_ERR_NO_MEM> Realloc size: %d, new_ptr: %p, heap free: %d", \
                 __func__, __LINE__, (int)size, new_ptr, esp_get_free_heap_size());             \
        return ESP_ERR_NO_MEM;                                                                  \
    }                                                                                           \
    new_ptr;                                                                                    \
})
...
#define ESP_RETURN_ON_NULL(ptr)                                     \
    ...
    {                                                               \
        if (!(ptr))                                                 \
        {                                                           \
            ESP_LOGW(TAG, "[%s, %d] <NULL POINTER> heap free: %d",  \
