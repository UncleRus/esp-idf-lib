    ...
    */
    ...
    while (1)
    {
        ...
        ESP_LOGI(TAG, "Acceleration: x: <%d> \t y: <%d> \t z: <%d>", data_accel.accel_x, data_accel.accel_y,
            data_accel.accel_z);
        ESP_LOGI(TAG, "    Rotation: x: <%d> \t y: <%d> \t z: <%d>", data_rotat.gyro_x, data_rotat.gyro_y, data_rotat.gyro_z);
        ESP_LOGI(TAG, "temperature %d", mpu6050_get_temperature(&(mpu6050_config)));
    }
    vTaskDelete(NULL);
}

void app_main() { xTaskCreate(mpu6050_test, "mpu6050_test", configMINIMAL_STACK_SIZE * 8, NULL, 5, NULL); }
